 ...
 */
public abstract class MultiEditorEditorPart extends EditorPart implements IPageChangeProvider {
	 ...
	 */
	protected MultiEditorEditorPart() {
		super();
	}
	 ...
	 *
	 * @see MultiEditorEditorPart#setControl(int, Control)
	 */
	 ...
	 *
	 * @see MultiEditorEditorPart#handlePropertyChange(int) the handler for property
	...
	public void addPage(int index, IEditorPart editor, IEditorInput input) throws PartInitException {
		...
		editor.addPropertyListener((source, propertyId) -> MultiEditorEditorPart.this.handlePropertyChange(propertyId));
	...
	protected IEditorSite createSite(IEditorPart editor) {
		return new MultiEditorEditorSite(this, editor);
	}
	...
	void close() {
		...
		if (widget != null && !widget.isDisposed()) {
			getSite().getPage().closeEditor(MultiEditorEditorPart.this, true);
		}
	}
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		setSite(site);
		setInput(input);
		site.setSelectionProvider(new MultiEditorSelectionProvider(this));
	}
	...
	protected void pageChange(int newPageIndex) {
		...
		if (contributor != null && contributor instanceof MultiEditorEditorActionBarContributor) {
			((MultiEditorEditorActionBarContributor) contributor).setActivePage(activeEditor);
		}
	 ...
	 * {@link MultiEditorEditorPart#createPageContainer(Composite)} to deactivate the
	 ...
	 * {@link MultiEditorEditorPart#pageChange(int)}.
	 * </p>
	 * <p>
	...
	private void disposePart(final IWorkbenchPart part) {
		SafeRunner.run(new ISafeRunnable() {
			...
			public void run() {
				...
				part.dispose();
				if (partSite instanceof MultiEditorEditorSite) {
					((MultiEditorEditorSite) partSite).dispose();
				}
			}
	 ...
	 *
	 * @param pageIndex the index of the page
	 * @see MultiEditorEditorPart#addPage(Control)
	 * @see MultiEditorEditorPart#addPage(IEditorPart, IEditorInput)
	 */
